...
class Reporter:
    ...
    def writeToXlsx(self, path=None, SD=None, dont_touch=None) -> None:
        ...
        for i in vuln_ids:
            ...
            if assets_without_critical_system == {}:
                ...
                for j in assets_without_critical_system:
                    sheet[f'D{num + 2}'] = f'{j} ({assets_without_critical_system[j][0]})'
                    solutions = vulnerabilities.vuln_solution(i, assets_without_critical_system[j][1])
                    sheet[f'C{num + 2}'] = str(solutions)
                    try:
        ...
        xlsx.save(f'{path}/{self.name}.xlsx')

    def create_report(self, report_type, path=None, SD=None, dont_touch=None) -> None:
        if report_type == 'xlsx':
            self.writeToXlsx(path, SD, dont_touch)
        else:
            print('unknown type')
